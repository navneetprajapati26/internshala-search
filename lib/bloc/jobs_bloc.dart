...
class InternshipBloc extends Bloc<InternshipEvent, InternshipState> {
  ...
  InternshipBloc() : super(InternshipState.initial()) {
    ...
    on<SearchByLocationNamesInternshipsEvent>(_onSearchByLocationNamesInternships);
    on<SearchByWorkFromHomeInternshipsEvent>(_onSearchByWorkFromHomeInternships);
  ...
  void _onSearchByLocationNamesInternships(SearchByLocationNamesInternshipsEvent event, Emitter<InternshipState> emit) {
    if (state.filteredInternships.isNotEmpty) {
      //log("filteredInternships state length " + state.filteredInternships.length.toString());

      final filteredInternships = jobsRepository.searchInternshipsByLocationNames(state.filteredInternships ?? [], event.locationNames);
      emit(state.copyWith(
          status: InternshipStatus.filteredInternships,
          filteredInternships: filteredInternships));
    } else {
  ...
  void _onSearchByWorkFromHomeInternships(SearchByWorkFromHomeInternshipsEvent event, Emitter<InternshipState> emit) {
    if (state.filteredInternships.isNotEmpty) {
      //log("filteredInternships state length " + state.filteredInternships.length.toString());

      final filteredInternships = jobsRepository.filterWorkFromHomeInternships(state.filteredInternships ?? []);
      emit(state.copyWith(
          status: InternshipStatus.filteredInternships,
          filteredInternships: filteredInternships));
    } else {
      ...
      final filteredInternships =
          jobsRepository.filterWorkFromHomeInternships(
              state.internships ?? []);
      emit(state.copyWith(
          status: InternshipStatus.filteredInternships,
          filteredInternships: filteredInternships));
  ...
  void _onEmptyFilterInternship(
    ...
    emit(state.copyWith(status: InternshipStatus.loaded, filteredInternships: []));
  }
}
